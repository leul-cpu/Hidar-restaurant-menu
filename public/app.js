document.addEventListener('DOMContentLoaded', () => {
    // URL parameters (table token)
    const urlParams = new URLSearchParams(window.location.search);
    const tableToken = urlParams.get('table') || '';

    // DOM Elements
    const qrGate = document.getElementById('qr-gate');
    ...
    function renderMenu(data) {
        ...
        data.categories.forEach(category => {
            ...
            category.items.forEach(item => {
                ...
                // Add controls or circular button based on availability
                let controlMarkup = '';
                if (item.available) {
                    const qty = cart[item.id] || 0;
                    if (qty > 0) {
                        controlMarkup = `
                            <div class="qty-controls" data-item-id="${item.id}">
                        ...
                        controlMarkup = `<button class="add-to-cart-btn" data-item-id="${item.id}">+</button>`;
                    }
                }
                ...
                // Wire up dynamic click events for cart buttons
                const addBtn = li.querySelector('.add-to-cart-btn');
                if (addBtn) {
                    addBtn.addEventListener('click', () => changeItemQuantity(item.id, 1));
                }

                const incBtn = li.querySelector('.inc-qty-btn');
                if (incBtn) {
                    incBtn.addEventListener('click', () => changeItemQuantity(item.id, 1));
                }

                const decBtn = li.querySelector('.dec-qty-btn');
                if (decBtn) {
                    decBtn.addEventListener('click', () => changeItemQuantity(item.id, -1));
                }
            ...
            category.items.forEach(item => {
                ...
                if (item.available) {
                    ...
                    if (qty > 0) {
                        controlMarkup = `
                            <div class="qty-controls">
                                <button class="qty-btn" onclick="window.adjustCart('${item.id}', -1)">−</button>
                                <span class="qty-val">${qty}</span>
                                <button class="qty-btn" onclick="window.adjustCart('${item.id}', 1)">+</button>
                            </div>
                        `;
                    } else {
                        controlMarkup = `<button class="add-to-cart-btn" onclick="window.adjustCart('${item.id}', 1)">+</button>`;
                    }
                }
